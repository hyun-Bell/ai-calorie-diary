...
import { mockFoodScenarios, defaultMockScenario } from './mock-data/food-analysis-scenarios';
...
export class OpenAIApiMockAdapter implements OpenAIApiPort {
  ...
  async analyzeFood(
    ...
    if (description.includes('에러') || description.includes('error')) {
      throw new Error('Mock API error for testing purposes');
    }

    // 시나리오 매칭
    const scenario = mockFoodScenarios.find(scenario =>
